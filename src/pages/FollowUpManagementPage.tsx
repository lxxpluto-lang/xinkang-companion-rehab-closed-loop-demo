...
import { AlertTriangle, CalendarClock, CheckCircle2, Clock3, MessageSquareText, Phone, Save, Search, UserRound, X } from "lucide-react";
...
function FollowUpModal({ task, patient, record, readOnly, currentAccount, role, onClose, onSave }: { task: FollowUpTask; patient: ManagedPatient; record?: FollowUpRecord; readOnly: boolean; currentAccount: string; role: "ADMIN" | "DOCTOR" | "REHAB_EXECUTION"; onClose: () => void; onSave: (record: FollowUpRecord) => void }) {
  ...
  const [communicationMethod, setCommunicationMethod] = useState<FollowUpRecord["communicationMethod"]>(record?.communicationMethod ?? "sms");
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 p-3 backdrop-blur-sm sm:p-6"><div className="flex max-h-[94vh] w-full max-w-5xl flex-col overflow-hidden rounded-xl bg-white shadow-2xl">
    ...
    <div className="overflow-y-auto px-5 py-5 sm:px-6">
      ...
      <div className="mt-4 flex flex-wrap items-center gap-2">{!readOnly && <><button type="button" onClick={() => setCommunicationMethod("sms")} className={communicationMethod === "sms" ? "btn-primary" : "btn-secondary"}><MessageSquareText className="h-4 w-4" />发送短信提醒</button><button type="button" onClick={() => setCommunicationMethod("online_message")} className={communicationMethod === "online_message" ? "btn-primary" : "btn-secondary"}><MessageSquareText className="h-4 w-4" />发送在线消息</button></>}{callablePhone && !readOnly ? <a href={`tel:${patient.phone}`} onClick={() => setCommunicationMethod("phone")} className="btn-secondary"><Phone className="h-4 w-4" />人工电话联系</a> : <span className="btn-secondary cursor-not-allowed opacity-60"><Phone className="h-4 w-4" />{callablePhone ? "只读查看" : "演示号码已脱敏"}</span>}<p className="w-full text-[10px] text-slate-400">验证版仅演示提醒渠道与记录流程，不接入 AI 自动外呼；电话入口仅发起系统拨号。</p></div>

      <div className="mt-5 rounded-xl border border-blue-200 bg-blue-50 p-4 text-xs leading-5 text-blue-900"><b>消息提醒与人工随访</b><p className="mt-1">患者不在患者端填写随访表。系统优先发送短信或在线消息；需要了解近况时，由康复师人工联系并根据患者口述记录，医生仅处理高风险升级事项。</p></div>
      ...
      <section className="mt-5"><SectionHeader title={readOnly ? "随访记录" : "本次提醒/沟通结果"} /><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3"><label><span className="field-label">联系结果</span><select disabled={readOnly} value={contactResult} onChange={(event) => setContactResult(event.target.value as ContactResult)} className="text-field disabled:bg-slate-50"><option value="reached">已联系/已送达</option><option value="no_answer">未接通/未确认</option><option value="refused">拒绝沟通</option><option value="invalid_number">号码异常</option></select></label><label><span className="field-label">沟通方式</span><select disabled={readOnly} value={communicationMethod} onChange={(event) => setCommunicationMethod(event.target.value as FollowUpRecord["communicationMethod"])} className="text-field disabled:bg-slate-50"><option value="sms">短信提醒</option><option value="online_message">在线消息</option><option value="phone">人工电话</option><option value="outpatient">门诊沟通</option><option value="other">其他</option></select></label><label><span className="field-label">实际沟通时间</span><input disabled={readOnly} type="datetime-local" value={contactedAt} onChange={(event) => setContactedAt(event.target.value)} className="text-field disabled:bg-slate-50" /></label></div></section>
