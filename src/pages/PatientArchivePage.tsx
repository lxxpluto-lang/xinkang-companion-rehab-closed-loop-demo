...
const patientProfiles = [
  { name: patientMasterChen.name, patient_no: patientMasterChen.patientNo, id_number: patientMasterChen.idNumber, phone: patientMasterChen.phone, rehab_group: patientMasterChen.rehabGroup, rehab_stage: "Ⅱ期 · 第4周", last_followup: patientMasterChen.latestFollowUp },
  { name: "李秀兰", patient_no: "000002", id_number: "3702********4826", phone: "136****1938", rehab_group: "运动康复 A 组", rehab_stage: "Ⅱ期 · 第2周", last_followup: "2026-07-26" },
  { name: "王先生", patient_no: "000003", id_number: "3702********7714", phone: "159****2850", rehab_group: "运动康复 B 组", rehab_stage: "Ⅱ期 · 第3周", last_followup: "2026-07-25" },
  { name: "赵女士", patient_no: "000004", id_number: "3702********3409", phone: "137****8246", rehab_group: "重点监护组", rehab_stage: "首次评估", last_followup: "2026-07-24" }
];
...
export function PatientArchivePage({
  ...
  const filteredPatients = useMemo(() => {
    ...
    return patients.filter((patient) => {
      const matchesKeyword = !value || [patient.name, patient.patient_no, patient.id_number, patient.phone, patient.diagnosis_summary].some((field) => field.toLowerCase().includes(value));
  ...
  function openCreate() {
    ...
    setEditDraft({
      ...
      patient_no: String(nextNumber).padStart(6, "0"),
      name: "",
      id_number: "",
      phone: "",
      age: 60,
          ...
          <section className="card overflow-hidden">
            <div className="flex flex-wrap items-center gap-2 border-b border-slate-100 p-4">
              <label className="relative block min-w-[280px] flex-1">
                ...
                <input value={keyword} onChange={(event) => setKeyword(event.target.value)} className="h-9 w-full rounded-lg border border-slate-200 pl-9 pr-3 text-xs outline-none focus:border-blue-400" placeholder="搜索姓名、患者号、证件号、电话或诊断" />
              </label>
            ...
            <div className="grid grid-cols-[0.72fr_0.76fr_0.56fr_1.2fr_0.5fr_0.72fr_0.68fr_0.7fr] bg-slate-50 px-5 py-2.5 text-[10px] font-bold text-slate-400">
              <span>患者姓名</span><span>患者号</span><span>性别/年龄</span><span>诊断摘要</span><span>风险</span><span>康复阶段</span><span>当前处方</span><span>操作</span>
            </div>
            {filteredPatients.map((patient) => (
              <div key={patient.patient_demo_id} className="grid grid-cols-[0.72fr_0.76fr_0.56fr_1.2fr_0.5fr_0.72fr_0.68fr_0.7fr] items-center border-t border-slate-100 px-5 py-3 text-xs hover:bg-blue-50">
                ...
                <span className="font-mono text-[10px] text-slate-500">{patient.patient_no}</span><span>{patient.gender} / {patient.age}岁</span><span className="pr-3 leading-5 text-slate-600">{patient.diagnosis_summary}</span>
...
function PatientDetail({ patient, activeTab, setActiveTab, tasks, onBack, onBackToPrescription, onEdit, onOpenPrescription }: {
  ...
  return (
    <div className="space-y-4">
      <PageHeader eyebrow="患者详情页面" title={`${patient.name} · ${patient.patient_no}`} description="以患者为中心聚合基础档案、医患沟通、历次处方、每次训练记录和报告。" action={<div className="flex gap-2">{onBackToPrescription && <button type="button" onClick={onBackToPrescription} className="btn-primary"><ArrowLeft className="h-4 w-4" />返回当前处方</button>}<button type="button" onClick={onBack} className="btn-secondary">返回患者列表</button></div>} />
      <section className="card p-4">
        <div className="flex items-start gap-4">
          ...
          <div className="grid flex-1 grid-cols-6 gap-3">
            <Summary label="患者号" value={patient.patient_no} /><Summary label="年龄 / 性别" value={`${patient.age}岁 / ${patient.gender}`} /><Summary label="危险分组" value={patient.risk_level} /><Summary label="康复阶段" value={patient.rehab_stage} /><Summary label="当前处方" value={patient.prescription_version} /><Summary label="训练状态" value={patient.training_status} />
          </div>
