...
import { getTodayPlan, type PrescriptionPlan } from "../prescriptionPlanData";
...
const workspaceTabs: { key: PrescriptionWorkspaceTab; label: string; icon: typeof UserRound }[] = [
  { key: "current", label: "纸质处方执行项目", icon: ClipboardCheck },
...
export function PrescriptionWorkspacePage({
  ...
  const [activeTab, setActiveTab] = useState<PrescriptionWorkspaceTab>(initialTab === "reports" ? "reports" : "current");
  ...
  const [sourceDrawer, setSourceDrawer] = useState<"profile" | "assessment" | "training" | "stage" | null>(null);
  ...
  const todayPlan = getTodayPlan(task.patientId);
  ...
  return (
    <section className="space-y-4 pb-24" data-testid="page-PRESCRIPTION-WORKSPACE">
      <header className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div className="flex flex-wrap items-start justify-between gap-4 border-b border-slate-100 px-5 py-4">
          <div className="flex items-start gap-3">
            ...
            <div><p className="text-[10px] font-bold tracking-wider text-blue-600">处方执行管理 · 患者工作区</p><h1 className="mt-1 text-xl font-bold text-slate-950">患者处方执行详情</h1><p className="mt-1 text-xs text-slate-500">{task.kind === "initial" ? "首次执行计划" : "阶段调整执行计划"} · {task.prescriptionNo} · {task.versionNo}</p></div>
          </div>
      ...
      <DecisionSummary task={task} profile={profileDraft} plan={todayPlan} assessment={assessment} training={patientTraining} safetyEvents={safetyEvents} onSource={setSourceDrawer} />

      <div className="flex items-start gap-3 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-xs leading-5 text-amber-900"><AlertTriangle className="mt-0.5 h-4 w-4 shrink-0 text-amber-600" /><div><b>系统边界提示：</b>完整诊断、药品医嘱和正式运动处方由院内 HIS/纸质处方完成。本页面只记录已确认的执行项目、训练数据、报告依据和患者可读提醒；AI 不自动诊断、不自动调方、不自动发布。</div></div>

      <nav className="grid overflow-hidden rounded-2xl border border-slate-200 bg-white p-1.5 shadow-sm lg:grid-cols-2" aria-label="处方执行与报告栏目">
      ...
      {activeTab === "current" && <ExecutionPlanTab task={task} plan={todayPlan} training={patientTraining} onReports={() => setActiveTab("reports")} />}
      ...
      {sourceDrawer && <SourceDrawer type={sourceDrawer} profile={profileDraft} assessment={assessment} training={patientTraining[0]} stageLabel={task.sourceLabel} onClose={() => setSourceDrawer(null)} />}
    </section>
  );
}

function ExecutionPlanTab({ task, plan, training, onReports }: { task: PrescriptionTask; plan: PrescriptionPlan; training: SingleTrainingReportDetail[]; onReports: () => void }) {
  const completed = training.filter((item) => item.status.includes("完成") || item.executionSummary.includes("完成")).length;
  const statusLabel = (status: PrescriptionPlan["items"][number]["status"]) => status === "completed" ? "已完成" : status === "in_progress" ? "进行中" : status === "skipped" ? "已跳过" : status === "partially_completed" ? "部分完成" : status === "interrupted" ? "已中断" : "待执行";
  return <div className="space-y-4"><section className="card overflow-hidden"><div className="flex flex-wrap items-start justify-between gap-3 border-b border-slate-100 bg-gradient-to-r from-blue-50 to-white px-5 py-4"><div><p className="text-[10px] font-bold text-blue-600">纸质/HIS处方的最小执行映射</p><h2 className="mt-1 text-lg font-bold text-slate-950">康复执行项目</h2><p className="mt-1 text-xs leading-5 text-slate-500">康复师对照院内正式处方选择本次执行项目，系统只记录训练项目、次数和设备数据，不重新开具处方。</p></div><div className="rounded-xl bg-white px-4 py-2 text-right shadow-sm"><p className="text-[9px] text-slate-400">关联编号</p><p className="mt-1 font-mono text-xs font-bold text-slate-800">{task.prescriptionNo}</p></div></div><div className="grid gap-3 p-5 lg:grid-cols-3">{plan.items.map((item) => <article key={item.itemId} className="rounded-2xl border border-slate-200 bg-white p-4"><div className="flex items-center justify-between"><span className="flex h-8 w-8 items-center justify-center rounded-xl bg-blue-50 text-xs font-bold text-blue-700">{item.order}</span><StatusBadge tone={item.status === "completed" ? "green" : item.status === "in_progress" ? "blue" : "gray"}>{statusLabel(item.status)}</StatusBadge></div><h3 className="mt-3 text-base font-bold text-slate-900">{item.label}</h3><p className="mt-1 text-xs text-slate-500">{item.mode} · {item.trainingMinutes} 分钟</p><div className="mt-3 grid grid-cols-2 gap-2 text-[10px]"><div className="rounded-lg bg-slate-50 p-2"><span className="text-slate-400">目标心率</span><b className="mt-1 block text-slate-700">{item.targetHeartRate ? `${item.targetHeartRate[0]}–${item.targetHeartRate[1]} bpm` : "按指导完成"}</b></div><div className="rounded-lg bg-slate-50 p-2"><span className="text-slate-400">目标强度</span><b className="mt-1 block text-slate-700">{item.targetRpe ? `RPE ${item.targetRpe[0]}–${item.targetRpe[1]}` : "未设置"}</b></div></div></article>)}</div><div className="flex flex-wrap items-center justify-between gap-3 border-t border-slate-100 bg-slate-50 px-5 py-4"><p className="text-xs text-slate-600">当前已形成 <b className="text-blue-700">{completed}</b> 条完成训练记录；项目完成状态由训练工作台和患者打卡回写。</p><button type="button" onClick={onReports} className="btn-primary"><FileText className="h-4 w-4" />查看训练记录与阶段报告</button></div></section><section className="rounded-2xl border border-amber-200 bg-amber-50 p-4 text-xs leading-6 text-amber-900"><b>验证版边界：</b>诊断、用药、完整运动强度和医生签名仍以院内正式系统或纸质处方为准。本系统不提供 AI 自动诊断、自动开方或自动发布。</section></div>;
}

function DecisionSummary({ task, profile, plan, assessment, training, safetyEvents, onSource }: { task: PrescriptionTask; profile: PatientClinicalProfile; plan: PrescriptionPlan; assessment: PatientClinicalProfile["rehabAssessment"]; training: SingleTrainingReportDetail[]; safetyEvents: typeof minimalSafetyEvents; onSource: (type: "profile" | "assessment" | "training" | "stage") => void }) {
  const completed = training.filter((item) => item.status.includes("完成") || item.executionSummary.includes("完成")).length;
  const latest = training[0];
  const metrics: Array<[string, string, "profile" | "assessment" | "training" | "stage"]> = [
    ["基础信息", `${profile.name} · ${profile.age}岁 · ${profile.sex}`, "profile" as const],
    ["康复依据", `${profile.rehabStage} · 出院 ${profile.dischargeDate || "未采集"}`, "profile" as const],
    ["CPET / 心率", `${cpetStatusLabel(profile.cpetStatus)} · AT ${metricText(profile.rehabAssessment.cpet.anaerobicThresholdHr)} · 峰值 ${metricText(profile.rehabAssessment.cpet.peakHr)} · 静息 ${metricText(profile.rehabAssessment.restingVitals.metric)}`, "assessment" as const],
    ["处方执行", `${completed}/${Math.max(3, training.length)} 次训练`, "training" as const],
    ["体能评估", assessment.status === "已复核" ? `SPPB ${assessment.sppb.balanceScore + assessment.sppb.gaitScore + assessment.sppb.chairStandScore}/12` : assessment.status, "assessment" as const],
    ["最新训练", latest ? `平均HR ${latest.hrStats.average} · SpO₂ ${latest.spo2Summary}` : "暂无单次报告", "training" as const],
    ["阶段依据", task.sourceLabel, "stage" as const]
  ];
  return <section className="rounded-2xl border border-blue-100 bg-gradient-to-r from-blue-50 via-white to-emerald-50 p-4 shadow-sm"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-[10px] font-bold tracking-wide text-blue-600">执行与报告摘要</p><h2 className="mt-1 text-base font-bold text-slate-950">先看训练结果，再核对执行项目</h2><p className="mt-1 text-[10px] text-slate-500">所有提示均可回到原始记录核对，不替代医生判断。</p></div><div className="rounded-xl bg-white px-3 py-2 text-right shadow-sm"><p className="text-[9px] text-slate-400">纸质处方执行项目</p><p className="mt-1 text-sm font-bold text-slate-900">{plan.items.map((item) => item.label).join(" → ") || "待同步"}</p></div></div><div className="mt-4 grid gap-2 md:grid-cols-5">{metrics.map(([label, value, source]) => <button type="button" key={label} onClick={() => onSource(source)} className="rounded-xl border border-white bg-white/90 p-3 text-left shadow-sm transition hover:border-blue-300"><p className="text-[9px] font-bold text-slate-400">{label}</p><p className="mt-1.5 line-clamp-2 text-xs font-bold leading-5 text-slate-800">{value}</p><span className="mt-2 block text-[9px] font-bold text-blue-600">查看来源 →</span></button>)}</div>{safetyEvents.length > 0 && <div className="mt-3 rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-[10px] font-semibold text-amber-800">近期有 {safetyEvents.length} 条安全事件，需要医生在报告中复核。</div>}</section>;
}

function cpetStatusLabel(status: PatientClinicalProfile["cpetStatus"]) {
...
function metricText(metric: { value: number | null; unit: string }) {
  return metric.value == null ? "未采集" : `${metric.value}${metric.unit}`;
}

function SourceDrawer({ type, profile, assessment, training, stageLabel, onClose }: { type: "profile" | "assessment" | "training" | "stage"; profile: PatientClinicalProfile; assessment: PatientClinicalProfile["rehabAssessment"]; training?: SingleTrainingReportDetail; stageLabel: string; onClose: () => void }) {
  const title = type === "profile" ? "患者基础信息来源" : type === "assessment" ? "体能评估来源" : type === "training" ? "最近单次训练来源" : "阶段报告来源";
  return <div className="fixed inset-0 z-50 flex justify-end bg-slate-950/30 backdrop-blur-[1px]"><aside className="h-full w-full max-w-md overflow-y-auto bg-white p-6 shadow-2xl"><div className="flex items-start justify-between"><div><p className="text-[10px] font-bold text-blue-600">来源核对</p><h2 className="mt-1 text-lg font-bold text-slate-950">{title}</h2></div><button type="button" onClick={onClose} className="rounded-lg p-2 text-slate-400 hover:bg-slate-100"><X className="h-4 w-4" /></button></div><div className="mt-5 space-y-3 text-xs">{type === "profile" && <><InfoRow label="病史" value={profile.medicalHistory} /><InfoRow label="诊断" value={profile.diagnosis} /><InfoRow label="特殊用药" value={profile.specialMedications} /><InfoRow label="出院/随访" value={`${profile.dischargeDate || "未采集"} · 下次 ${profile.nextFollowUpDate || "未安排"}`} /></>}{type === "assessment" && <><InfoRow label="评估状态" value={assessment.status} /><InfoRow label="CPET状态" value={cpetStatusLabel(profile.cpetStatus)} /><InfoRow label="无氧阈心率" value={`${metricText(assessment.cpet.anaerobicThresholdHr)} · ${assessment.cpet.anaerobicThresholdHr.measuredAt}`} /><InfoRow label="峰值心率" value={`${metricText(assessment.cpet.peakHr)} · ${assessment.cpet.peakHr.measuredAt}`} /><InfoRow label="静息心率" value={`${metricText(assessment.restingVitals.metric)} · ${assessment.restingVitals.metric.measuredAt}`} /><InfoRow label="SPPB" value={`${assessment.sppb.balanceScore + assessment.sppb.gaitScore + assessment.sppb.chairStandScore}/12`} /><InfoRow label="6分钟步行" value={`${assessment.sixMinuteWalk.distanceMeters ?? "未采集"} m`} /><InfoRow label="CPET峰值VO₂" value={`${assessment.cpet.peakVo2 ?? "未采集"} ml/kg/min`} /></>}{type === "training" && <>{training ? <><InfoRow label="训练时间" value={formatDateTime(training.actualStartAt)} /><InfoRow label="实际运动" value={`${training.activeMinutes} 分钟`} /><InfoRow label="实际心率" value={`平均 ${training.hrStats.average} / 峰值 ${training.hrStats.peak} bpm`} /><InfoRow label="血压测量" value={training.bpMeasurements.map((item) => `${item.phase} ${item.time} ${item.value}`).join("；") || "未采集"} /></> : <InfoRow label="最近训练" value="暂无单次报告" />}</>}{type === "stage" && <InfoRow label="阶段报告" value={`${stageLabel}；请在训练记录与阶段报告 Tab 查看完整版本比较。`} />}</div><button type="button" onClick={onClose} className="mt-6 w-full btn-primary">返回处方工作区</button></aside></div>;
}

function InfoRow({ label, value }: { label: string; value: string }) { return <div className="rounded-xl border border-slate-100 bg-slate-50 p-3"><p className="text-[9px] font-bold text-slate-400">{label}</p><p className="mt-1 leading-5 text-slate-700">{value}</p></div>; }
    ...
    <div className="space-y-3">
      <section className="card overflow-hidden">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 bg-gradient-to-r from-blue-50 to-white px-4 py-3">
          <SectionHeader title="本次口述补充（可选）" description="HIS/纸质病历已有内容时无需重复录入；仅补充会影响本次执行计划的变化。" action={<StatusBadge tone={saved ? "green" : "orange"}>{saved ? "已保存" : dirty ? "正在记录" : "可选记录"}</StatusBadge>} />
        ...
        <div className="p-4">
          ...
          <p className="mt-2 text-[9px] text-slate-400">口述记录来自医生面谈或康复师电话转述；未填写不会阻断查看训练报告，出现异常时请补充说明。</p>
        </div>
      </section>
      <section className="card overflow-hidden">
...
function CurrentPrescriptionTab({ task, content, previousVersion, safetyEvents, readonly, narrativeSaved, assessmentSummary, profileComplete, previousReviewed, riskReviewed, contentConfirmed, signatureBlockers, onGenerate, onFieldChange, onOpenCarry, onPreviousReviewed, onRiskReviewed, onContentConfirmed, onSign, onPrint }: { task: PrescriptionTask; content: PrescriptionContent; previousVersion: PrescriptionVersionDetail; safetyEvents: typeof minimalSafetyEvents; readonly: boolean; narrativeSaved: boolean; assessmentSummary: string; profileComplete: boolean; previousReviewed: boolean; riskReviewed: boolean; contentConfirmed: boolean; signatureBlockers: string[]; onGenerate: () => void; onFieldChange: <K extends keyof PrescriptionContent>(key: K, value: PrescriptionContent[K]) => void; onOpenCarry: () => void; onPreviousReviewed: (value: boolean) => void; onRiskReviewed: (value: boolean) => void; onContentConfirmed: (value: boolean) => void; onSign: () => void; onPrint: () => void }) {
  ...
  return <div className="space-y-4"><AiWorkspace task={task} previous={previousVersion} content={content} readonly={readonly} narrativeSaved={narrativeSaved} onGenerate={onGenerate} /><section className="card overflow-hidden"><div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 px-5 py-4"><SectionHeader title="本次执行计划" description={readonly ? "执行计划已确认，当前只能查看和打印。" : "请根据 HIS/纸质处方维护本次康复执行项目；本页面不自动生成临床医嘱。"} action={<span className="text-[10px] text-slate-400">关联处方号 {task.prescriptionNo}</span>} /><div className="flex gap-2">{!readonly && task.kind === "adjustment" && <button type="button" className="btn-secondary" onClick={onOpenCarry}><History className="h-4 w-4" />从上一版逐项沿用</button>}{readonly && <button type="button" className="btn-primary" onClick={onPrint}><Printer className="h-4 w-4" />查看/打印</button>}</div></div><div className="space-y-4 bg-slate-50/60 p-5"><OptionSection title="康复目标" options={["降低血压", "降低血脂", "改善症状", "提高体能", "改善心功能", "改善睡眠", "预防支架内再狭窄"]} selected={content.rehabGoals} disabled={readonly} onChange={(value) => onFieldChange("rehabGoals", value)} /><ExerciseSection title="呼吸训练" modes={content.breathingModes} modeOptions={["腹式呼吸练习", "吸气抬手", "吸气耸肩", "正念呼吸"]} intensity={content.breathingIntensity} frequency={content.breathingFrequency} time={content.breathingTime} disabled={readonly} onModes={(value) => onFieldChange("breathingModes", value)} onIntensity={(value) => onFieldChange("breathingIntensity", value)} onFrequency={(value) => onFieldChange("breathingFrequency", value)} onTime={(value) => onFieldChange("breathingTime", value)} /><ExerciseSection title="热身运动" modes={content.warmupModes} modeOptions={["原地踏步", "肩部热身运动", "扩胸运动", "四肢伸展运动", "手腕踝关节"]} intensity="低强度，逐步提升心率" frequency={content.warmupFrequency} time={content.warmupTime} disabled={readonly} onModes={(value) => onFieldChange("warmupModes", value)} onIntensity={() => undefined} onFrequency={(value) => onFieldChange("warmupFrequency", value)} onTime={(value) => onFieldChange("warmupTime", value)} /><ExerciseSection title="有氧运动" modes={content.aerobicModes} modeOptions={["步行", "骑自行车", "功率车", "椭圆机", "八段锦", "太极拳"]} intensity={content.aerobicIntensity} frequency={content.aerobicFrequency} time={content.aerobicTime} disabled={readonly} onModes={(value) => onFieldChange("aerobicModes", value)} onIntensity={(value) => onFieldChange("aerobicIntensity", value)} onFrequency={(value) => onFieldChange("aerobicFrequency", value)} onTime={(value) => onFieldChange("aerobicTime", value)} /><ExerciseSection title="抗阻训练" modes={content.resistanceModes} modeOptions={["哑铃", "弹力带", "绑腿沙袋", "下肢静蹲"]} intensity={content.resistanceIntensity} frequency={content.resistanceFrequency} time={content.resistanceTime} disabled={readonly} onModes={(value) => onFieldChange("resistanceModes", value)} onIntensity={(value) => onFieldChange("resistanceIntensity", value)} onFrequency={(value) => onFieldChange("resistanceFrequency", value)} onTime={(value) => onFieldChange("resistanceTime", value)} /><ExerciseSection title="柔韧性训练" modes={content.flexibilityModes} modeOptions={["颈部肌肉牵伸", "躯干肌肉牵伸", "上肢肌肉牵伸", "下肢肌肉牵伸"]} intensity={content.flexibilityIntensity} frequency={content.flexibilityFrequency} time={content.flexibilityTime} disabled={readonly} onModes={(value) => onFieldChange("flexibilityModes", value)} onIntensity={(value) => onFieldChange("flexibilityIntensity", value)} onFrequency={(value) => onFieldChange("flexibilityFrequency", value)} onTime={(value) => onFieldChange("flexibilityTime", value)} /><EditArea label="备注与注意事项" value={content.remark} disabled={readonly} onChange={(value) => onFieldChange("remark", value)} /><section className="rounded-2xl border border-amber-100 bg-amber-50/70 p-4"><SectionHeader title="患者可读的康复提醒" description="以下内容将同步到患者端和打印处方。" /><div className="grid gap-3 md:grid-cols-2"><EditArea label="诊断建议" value={content.diagnosisAdvice} disabled={readonly} onChange={(value) => onFieldChange("diagnosisAdvice", value)} /><EditArea label="用药提醒" value={content.medicationAdvice} disabled={readonly} onChange={(value) => onFieldChange("medicationAdvice", value)} /><EditArea label="康复期间忌讳" value={content.rehabContraindications} disabled={readonly} onChange={(value) => onFieldChange("rehabContraindications", value)} /><EditArea label="饮食注意" value={content.dietCautions} disabled={readonly} onChange={(value) => onFieldChange("dietCautions", value)} /><EditArea label="运动注意" value={content.exerciseCautions} disabled={readonly} onChange={(value) => onFieldChange("exerciseCautions", value)} /><EditArea label="停止运动条件" value={content.stopConditions} disabled={readonly} onChange={(value) => onFieldChange("stopConditions", value)} /><div className="md:col-span-2"><EditArea label="给患者的一句话说明" value={content.patientInstruction} disabled={readonly} onChange={(value) => onFieldChange("patientInstruction", value)} /></div></div></section></div></section><section className="card p-5"><SectionHeader title="确认与发布前核对" description="所有必检项完成后，医生才能确认执行计划并签署。" action={<ClipboardCheck className="h-4 w-4 text-blue-600" />} /><div className="mt-4 grid gap-3 lg:grid-cols-3"><ChecklistItem checked={narrativeSaved} label="本次问诊已保存" fixed /><ChecklistItem checked={profileComplete} label="关键临床资料完整" fixed /><ChecklistItem checked={previousReviewed || task.kind === "initial"} label={task.kind === "initial" ? "首次计划无需核对上一版" : "已核对上一版执行计划"} disabled={readonly || task.kind === "initial"} onChange={onPreviousReviewed} /><ChecklistItem checked={!safetyEvents.length || riskReviewed} label={safetyEvents.length ? "已核对近期异常与处置" : "无近期安全事件"} disabled={readonly || !safetyEvents.length} onChange={onRiskReviewed} /><ChecklistItem checked={riskReviewed || task.risk !== "高危"} label={task.risk === "高危" ? "已完成高危患者人工复核" : "风险等级已确认"} disabled={readonly || task.risk !== "高危"} onChange={onRiskReviewed} /><ChecklistItem checked={contentConfirmed} label="本次执行计划由医生确认" disabled={readonly} onChange={onContentConfirmed} /></div><div className="mt-4 flex justify-end">{!readonly && <button type="button" className="btn-primary" onClick={onSign}><BadgeCheck className="h-4 w-4" />确认完成并数字签署</button>}</div></section></div>;
}
...
function AiWorkspace({ task, previous, content, readonly, narrativeSaved, assessmentSummary = "康复评估结果与自动评分", onGenerate }: { task: PrescriptionTask; previous: PrescriptionVersionDetail; content: PrescriptionContent; readonly: boolean; narrativeSaved: boolean; assessmentSummary?: string; onGenerate: () => void }) {
  ...
  void assessmentSummary;
  void onGenerate;
  return <section className="flex flex-wrap items-center gap-3 rounded-2xl border border-blue-200 bg-gradient-to-r from-blue-50 via-white to-emerald-50 px-4 py-3 shadow-sm" data-testid="region-AI-REPORT-ASSIST"><span className="rounded-xl bg-blue-600 p-2.5 text-white"><Sparkles className="h-4 w-4" /></span><div className="min-w-0 flex-1"><div className="flex items-center gap-2"><h2 className="text-sm font-bold text-slate-900">AI 阶段报告分析</h2><StatusBadge tone="blue">仅供医生参考</StatusBadge></div><p className="mt-1 text-[10px] leading-5 text-slate-500">本 Demo 不使用 AI 自动诊断或自动开方。完成训练后可生成阶段报告摘要，医生仍需核对原始数据并手动确认处方。</p></div><span className="rounded-lg border border-emerald-200 bg-emerald-50 px-3 py-2 text-[10px] font-bold text-emerald-700">处方来源：HIS/纸质处方同步</span></section>;
}
      ...
      <section className="mx-auto flex max-h-[92vh] max-w-4xl flex-col overflow-hidden rounded-2xl bg-white shadow-2xl">
        ...
        <div className="overflow-y-auto bg-slate-100 p-6">
          <article className="mx-auto min-h-[760px] max-w-3xl bg-white p-8 text-xs leading-6 shadow-sm" data-testid="formal-prescription-document">
            ...
            <PrescriptionLine title="患者注意事项" value={`诊断建议：${content.diagnosisAdvice}；用药提醒：${content.medicationAdvice}；康复期间忌讳：${content.rehabContraindications}；饮食注意：${content.dietCautions}；运动注意：${content.exerciseCautions}；停止条件：${content.stopConditions}；${content.patientInstruction}`} />
