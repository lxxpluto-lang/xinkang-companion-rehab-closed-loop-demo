...
function CurrentPrescriptionTab({ task, content, finalDraft, editable, responsibilityConfirmed, onResponsibilityConfirmed, onGenerate, onDraftChange, onContentChange, onSave, onConfirm }: { task: PrescriptionTask; content: PrescriptionContent; finalDraft?: PrescriptionDraft; editable: boolean; responsibilityConfirmed: boolean; onResponsibilityConfirmed: (value: boolean) => void; onGenerate: () => void; onDraftChange: (draft: PrescriptionDraft) => void; onContentChange: <K extends keyof PrescriptionContent>(key: K, value: PrescriptionContent[K]) => void; onSave: () => void; onConfirm: () => void }) {
  ...
  return <div className="space-y-4">
    <section className="rounded-2xl border border-blue-200 bg-blue-50 p-4"><div className="flex flex-wrap items-center justify-between gap-3"><div className="flex items-start gap-3"><span className="rounded-xl bg-blue-600 p-2.5 text-white"><Sparkles className="h-4 w-4" /></span><div><h2 className="text-sm font-bold text-blue-950">AI 辅助处方草稿</h2><p className="mt-1 text-sm leading-5 text-blue-800">AI只生成建议草稿，不改变诊断、危险分组和正式参数，不自动签署或发布。</p>{task.aiSuggestionMeta && <p className="mt-2 text-xs text-blue-700">生成依据：{task.aiSuggestionMeta.sourceRecordIds.length}条患者记录{task.aiSuggestionMeta.missingFields.length ? ` · 缺失：${task.aiSuggestionMeta.missingFields.join("、")}` : " · 来源完整"}</p>}</div></div>{!ai && editable && <button type="button" className="btn-primary" onClick={onGenerate}><Bot className="h-4 w-4" />生成 AI 处方草稿</button>}</div></section>
