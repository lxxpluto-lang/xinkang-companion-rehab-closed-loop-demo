...
function DecisionSummary({ task, profile, plan, assessment, training, safetyEvents, onSource }: { task: PrescriptionTask; profile: PatientClinicalProfile; plan: PrescriptionPlan; assessment: PatientClinicalProfile["rehabAssessment"]; training: SingleTrainingReportDetail[]; safetyEvents: typeof minimalSafetyEvents; onSource: (type: "profile" | "assessment" | "training" | "stage") => void }) {
  ...
  const metrics: Array<[string, string, "profile" | "assessment" | "training" | "stage"]> = [
    ["基础信息", `${profile.name} · ${profile.age}岁 · ${profile.riskLevel}`, "profile" as const],
  ...
  return <section className="rounded-2xl border border-blue-100 bg-gradient-to-r from-blue-50 via-white to-emerald-50 p-4 shadow-sm"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-[10px] font-bold tracking-wide text-blue-600">处方决策摘要</p><h2 className="mt-1 text-base font-bold text-slate-950">先看结论，再编辑处方</h2><p className="mt-1 text-[10px] text-slate-500">所有提示均可回到原始记录核对，不替代医生判断。</p></div><div className="rounded-xl bg-white px-3 py-2 text-right shadow-sm"><p className="text-[9px] text-slate-400">本次处方项目</p><p className="mt-1 text-sm font-bold text-slate-900">{plan.items.map((item) => item.label).join(" → ") || "待配置"}</p></div></div><div className="mt-4 grid gap-2 md:grid-cols-5">{metrics.map(([label, value, source]) => <button type="button" key={label} onClick={() => onSource(source)} className="rounded-xl border border-white bg-white/90 p-3 text-left shadow-sm transition hover:border-blue-300"><p className="text-[9px] font-bold text-slate-400">{label}</p><p className="mt-1.5 line-clamp-2 text-xs font-bold leading-5 text-slate-800">{value}</p><span className="mt-2 block text-[9px] font-bold text-blue-600">查看来源 →</span></button>)}</div>{safetyEvents.length > 0 && <div className="mt-3 rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-[10px] font-semibold text-amber-800">近期有 {safetyEvents.length} 条安全事件，签署前必须完成异常复核。</div>}</section>;
}
