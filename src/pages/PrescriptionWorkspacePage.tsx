...
export function PrescriptionWorkspacePage({
  ...
  const hasProfileMissing = [profileDraft.contact, profileDraft.diagnosis, profileDraft.medicalHistory, profileDraft.specialMedications, profileDraft.cpet, profileDraft.sixMinuteWalk].some((value) => !value.trim() || value.includes("待补充"));
    ...
    <section className="space-y-4 pb-24" data-testid="page-PRESCRIPTION-WORKSPACE">
      <header className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div className="flex flex-wrap items-start justify-between gap-4 border-b border-slate-100 px-5 py-4">
          ...
          <div className="flex flex-wrap items-center gap-2"><StatusBadge tone={task.status === "completed" ? "green" : "orange"}>{prescriptionStatusLabels[task.status]}</StatusBadge><button type="button" onClick={openFullArchive} className="btn-secondary"><UserRound className="h-4 w-4" />查看完整患者档案</button></div>
        </div>
...
function ProfileTab({ profile, readonly, dirty, onChange, onSave }: { profile: PatientClinicalProfile; readonly: boolean; dirty: boolean; onChange: <K extends keyof PatientClinicalProfile>(key: K, value: PatientClinicalProfile[K]) => void; onSave: () => void }) {
  return <section className="card overflow-hidden"><div className="flex items-center justify-between border-b border-slate-100 px-5 py-4"><SectionHeader title="患者基本信息与临床资料" description="身份主索引只读；医生可维护与本次开方相关的临床信息。" /><div className="flex items-center gap-2">{profile.auditSummary && <span className="text-[10px] text-slate-400">{profile.auditSummary} · {profile.updatedBy}</span>}{!readonly && <button type="button" onClick={onSave} disabled={!dirty} className="btn-primary"><Save className="h-4 w-4" />保存临床资料</button>}</div></div><div className="grid gap-4 p-5 md:grid-cols-2 xl:grid-cols-4"><ReadOnlyField label="患者号" value={profile.patientNo} /><ReadOnlyField label="患者姓名" value={profile.name} /><ReadOnlyField label="证件信息（脱敏）" value={profile.idNumberMasked} /><ReadOnlyField label="性别 / 年龄" value={`${profile.sex} / ${profile.age}岁`} /><EditField label="联系电话" value={profile.contact} disabled={readonly} onChange={(value) => onChange("contact", value)} /><SelectField label="危险分组" value={profile.riskLevel} disabled={readonly} options={["低危", "中危", "高危"]} onChange={(value) => onChange("riskLevel", value as PatientClinicalProfile["riskLevel"])} /><EditField label="康复阶段" value={profile.rehabStage} disabled={readonly} onChange={(value) => onChange("rehabStage", value)} /><EditField label="静息生命体征" value={profile.restingVitals} disabled={readonly} onChange={(value) => onChange("restingVitals", value)} /><EditField label="CPET" value={profile.cpet} disabled={readonly} onChange={(value) => onChange("cpet", value)} /><EditField label="6分钟步行" value={profile.sixMinuteWalk} disabled={readonly} onChange={(value) => onChange("sixMinuteWalk", value)} /><EditField label="特殊用药" value={profile.specialMedications} disabled={readonly} onChange={(value) => onChange("specialMedications", value)} /><div className="md:col-span-2 xl:col-span-4"><EditArea label="诊断摘要" value={profile.diagnosis} disabled={readonly} onChange={(value) => onChange("diagnosis", value)} /></div><div className="md:col-span-2 xl:col-span-4"><EditArea label="既往史与风险说明" value={profile.medicalHistory} disabled={readonly} onChange={(value) => onChange("medicalHistory", value)} /></div></div></section>;
}
...
function CurrentPrescriptionTab({ task, content, previousVersion, safetyEvents, readonly, narrativeSaved, profileComplete, previousReviewed, riskReviewed, contentConfirmed, signatureBlockers, onGenerate, onFieldChange, onOpenCarry, onPreviousReviewed, onRiskReviewed, onContentConfirmed, onSign, onPrint }: { task: PrescriptionTask; content: PrescriptionContent; previousVersion: PrescriptionVersionDetail; safetyEvents: typeof minimalSafetyEvents; readonly: boolean; narrativeSaved: boolean; profileComplete: boolean; previousReviewed: boolean; riskReviewed: boolean; contentConfirmed: boolean; signatureBlockers: string[]; onGenerate: () => void; onFieldChange: <K extends keyof PrescriptionContent>(key: K, value: PrescriptionContent[K]) => void; onOpenCarry: () => void; onPreviousReviewed: (value: boolean) => void; onRiskReviewed: (value: boolean) => void; onContentConfirmed: (value: boolean) => void; onSign: () => void; onPrint: () => void }) {
...
function AiWorkspace({ task, previous, content, readonly, narrativeSaved, onGenerate }: { task: PrescriptionTask; previous: PrescriptionVersionDetail; content: PrescriptionContent; readonly: boolean; narrativeSaved: boolean; onGenerate: () => void }) {
  const [expanded, setExpanded] = useState(false);
  const draft = task.aiDraft;
  const evidence = draft ? Array.from(new Set([...draft.evidenceSnapshot, narrativeSaved ? "本次问诊记录" : "本次问诊尚未保存", "近期训练完成情况与安全事件"])) : [];
