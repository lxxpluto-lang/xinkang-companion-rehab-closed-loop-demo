...
import type { AlertDoctorDecision, AlertEvent, AlertRule, AlertSeverity, AlertStatus, PrescriptionTask } from "../clinicalWorkflowData";
...
const doctorDecisionLabel: Record<AlertDoctorDecision, string> = { no_change: "无需调整", adjust_prescription: "调整处方", pause_training: "暂停训练", reassess: "要求重新评估" };
...
export function AlertManagementPage({ role, accountId, initialStatus = "all", patients, prescriptionTasks, events, setEvents, rules, setRules }: {
  ...
  function saveAndComplete() {
    if (!selected?.doctorConclusion?.trim() || !selected.doctorDecision || !canReview || selected.status === "closed") return;
    setEvents((items) => items.map((item) => item.id === selected.id ? { ...item, status: "closed", doctorReviewedBy: accountId, doctorReviewedAt: new Date().toISOString() } : item));
    setSelectedId(null);
  }
