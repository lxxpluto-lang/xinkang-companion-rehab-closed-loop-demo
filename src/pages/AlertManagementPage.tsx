...
export function AlertManagementPage({ role, accountId, initialStatus = "all", patients, prescriptionTasks, events, setEvents, rules, setRules }: {
  ...
  function saveAndComplete() {
    if (!selected?.doctorConclusion?.trim() || role !== "DOCTOR") return;
  ...
  return <section data-testid="page-VIEW-ALERTS">
    ...
    {selected && <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 p-6" onMouseDown={() => { setSelectedId(null); setAiOpen(false); }}><article className="w-full max-w-2xl rounded-2xl bg-white p-6 shadow-2xl" onMouseDown={(event) => event.stopPropagation()}><div className="flex items-center justify-between"><div><p className="text-xs font-bold text-red-600">{selected.id} · {severityLabel[selected.severity]}</p><h2 className="mt-1 text-lg font-bold">{selected.patientName} · {selected.type}</h2></div><button onClick={() => { setSelectedId(null); setAiOpen(false); }}><X className="h-5 w-5" /></button></div>{aiOpen ? <div className="mt-5 rounded-2xl border border-blue-200 bg-blue-50 p-5"><h3 className="flex items-center gap-2 text-sm font-bold text-blue-900"><Bot className="h-5 w-5" />AI辅助解读</h3><p className="mt-3 text-xs leading-6 text-blue-900">该记录触发“{selected.type}”规则。建议结合运动阶段、患者症状、用药、设备质量和复测结果综合判断；优先确认传感器连接并按现场流程暂停或降低强度。</p><p className="mt-3 border-t border-blue-200 pt-3 text-xs font-bold text-blue-700">辅助信息，不替代医生判断，不生成诊断或处方。</p></div> : <div className="mt-5 space-y-4"><div className="rounded-xl bg-slate-50 p-4 text-xs leading-6"><b>触发快照：</b>{selected.snapshot}</div><label><span className="field-label">康复师现场记录</span><textarea disabled={role !== "REHAB_EXECUTION" || selected.status === "closed"} className="text-field min-h-24" value={selected.onSiteRecord ?? ""} onChange={(event) => updateEvent({ onSiteRecord: event.target.value, status: "processing" })} placeholder="记录暂停、复测与现场处置" /></label><label><span className="field-label">医生处理结论</span><textarea disabled={role !== "DOCTOR" || selected.status === "closed"} className="text-field min-h-24" value={selected.doctorConclusion ?? ""} onChange={(event) => updateEvent({ doctorConclusion: event.target.value })} placeholder="填写复核判断和后续处理意见" /></label><div className="flex items-center justify-between"><StatusBadge tone={selected.status === "closed" ? "green" : "orange"}>{statusLabel[selected.status]}</StatusBadge><div className="flex gap-3">{role === "REHAB_EXECUTION" && selected.status !== "closed" && <button className="btn-primary" disabled={!selected.onSiteRecord?.trim()} onClick={() => updateEvent({ status: "pending_doctor_review" })}>保存并提交医生</button>}{role === "DOCTOR" && selected.status !== "closed" && <button className="btn-primary" disabled={!selected.doctorConclusion?.trim()} onClick={saveAndComplete}><CheckCircle2 className="h-4 w-4" />保存并完成</button>}</div></div></div>}</article></div>}
  </section>;
}
