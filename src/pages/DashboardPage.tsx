  ...
  if (role === "REHAB_EXECUTION") {
    ...
    return <section data-testid="page-VIEW-DASHBOARD">
      ...
      <div className="grid gap-5 lg:grid-cols-[1.2fr_0.8fr]">
        ...
        <section className="card p-5"><SectionHeader title="现场与今日安排" description="训练、预约、随访和异常均从当天任务进入。" /><div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-1"><DashboardShortcut title="进入训练大屏" detail="查看到诊、在训、完成和异常状态" icon={MonitorUp} onClick={onOpenTraining} /><DashboardShortcut title="今日预约" detail={`${todayAppointments.length} 人待到诊或训练`} icon={CalendarDays} onClick={() => onNavigate("appointments")} /><DashboardShortcut title="到期随访" detail={`${pendingFollowUps.length} 项待联系`} icon={PhoneCall} tone="green" onClick={() => onOpenFollowUps("pending")} /><DashboardShortcut title="现场异常记录" detail={`${pendingAlerts.filter((item) => item.status === "pending").length} 条待上报`} icon={AlertTriangle} tone="red" onClick={() => onNavigate("alerts")} /></div><p className="mt-4 rounded-xl bg-slate-50 p-3 text-xs leading-5 text-slate-500">康复师可填写体能评估和治疗记录；不能编辑医生处方、诊断或危险分组。</p></section>
      </div>
    </section>;
  }
