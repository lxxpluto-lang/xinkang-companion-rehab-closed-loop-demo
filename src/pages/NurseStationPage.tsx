...
export function NurseStationPage({ role }: { role: Exclude<Role, "PATIENT"> }) {
  ...
  const [workflowStep, setWorkflowStep] = useState<"select" | "device" | "training" | "post" | "reported">("select");
      ...
      <div className="grid grid-cols-[1.12fr_0.88fr] gap-5">
        ...
        <div className="space-y-4">
          <section className="card p-4">
            <div className="flex items-center gap-4"><span className="flex h-12 w-12 items-center justify-center rounded-2xl bg-blue-50 text-blue-700"><UserCheck className="h-5 w-5" /></span><div className="min-w-0 flex-1"><p className="text-[10px] font-bold text-blue-600">当前患者与训练来源</p><h3 className="mt-1 text-sm font-bold text-slate-900">{displayPatient(selected)} · {exerciseRecords[selected.id]?.[0] ?? selected.exercise}</h3><p className="mt-1 text-[10px] text-slate-500">{projectSource[selected.id] === "his_reference" ? "来源：已签署处方/院内系统" : projectSource[selected.id] === "patient_material" ? "来源：当日预约或患者材料" : "来源：康复师现场补充"} · 历史实际完成 {actualCompletedCount[selected.id] ?? 0} 次</p></div><button type="button" disabled={!canExecute} onClick={() => { setSavedSelection(selected.id); setWorkflowStep("device"); window.setTimeout(() => setSavedSelection(null), 1600); }} className="btn-primary disabled:cursor-not-allowed disabled:bg-slate-300">进入设备检查</button></div>
            {savedSelection === selected.id && <p className="mt-3 rounded-lg border border-emerald-100 bg-emerald-50 p-3 text-xs font-bold text-emerald-700">已建立本次执行记录，下一步检查训练设备。</p>}
          </section>
          {canExecute && workflowStep !== "select" && <ExecutionWorkflow step={workflowStep} setStep={(next) => { if (next === "reported" && workflowStep !== "reported") setActualCompletedCount((items) => ({ ...items, [selected.id]: (items[selected.id] ?? 0) + 1 })); setWorkflowStep(next); }} postVitals={postVitals} setPostVitals={setPostVitals} rpe={rpe} setRpe={setRpe} />}
          <section className="card overflow-hidden">
...
function ExecutionWorkflow({ step, setStep, postVitals, setPostVitals, rpe, setRpe }: {
  step: "select" | "device" | "training" | "post" | "reported";
  setStep: (step: "select" | "device" | "training" | "post" | "reported") => void;
  ...
  const order = ["device", "training", "post", "reported"] as const;
  const labels = ["设备连接", "训练执行", "训练后记录", "单次报告"];
  const activeIndex = order.indexOf(step as typeof order[number]);
  return <section className="card p-4">
    <div className="grid grid-cols-4 gap-2">{labels.map((label, index) => <div key={label} className={`rounded-lg px-2 py-2 text-center text-[10px] font-bold ${index <= activeIndex ? "bg-blue-600 text-white" : "bg-slate-100 text-slate-400"}`}>{index + 1}. {label}</div>)}</div>
    ...
    {step === "reported" && <div className="mt-4 rounded-xl border border-emerald-200 bg-emerald-50 p-4"><div className="flex items-center justify-between"><div><b className="text-sm text-emerald-900">单次报告已自动生成</b><p className="mt-1 text-xs text-emerald-700">已记录设备指标、训练后生命体征、RPE、异常和处置；本次计入实际累计次数，不代表处方完成进度。</p></div><StatusBadge tone="green">待康复师确认</StatusBadge></div></div>}
  </section>;
}
