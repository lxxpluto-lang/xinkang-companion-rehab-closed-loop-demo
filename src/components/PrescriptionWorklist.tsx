...
export function PrescriptionWorklist({
  ...
  const filtered = useMemo(() => tasks.filter((task) => {
    const matchesQuery = `${task.patientName}${task.patientId}`.toLowerCase().includes(query.toLowerCase());
    ...
    <section className="card overflow-hidden" data-testid="region-PRESCRIPTION-WORKLIST">
      ...
      <div className="flex flex-wrap items-center gap-2 border-b border-slate-100 bg-slate-50/60 px-5 py-3">
        <label className="relative min-w-[210px] flex-1">
          ...
          <input value={query} onChange={(event) => setQuery(event.target.value)} className="h-9 w-full rounded-lg border border-slate-200 bg-white pl-9 pr-3 text-xs outline-none focus:border-blue-400" placeholder="搜索患者姓名或编号" />
        </label>
      ...
      <div className="overflow-x-auto">
        ...
        {filtered.map((task) => (
          <div key={task.id} className="grid min-w-[1050px] grid-cols-[1.1fr_0.9fr_0.72fr_1.18fr_0.72fr_0.72fr_0.85fr_0.9fr] items-center border-t border-slate-100 px-5 py-3 text-xs hover:bg-blue-50/30">
            <div><p className="font-bold text-slate-900">{task.patientName}</p><p className="mt-1 text-[10px] text-slate-400">{task.patientId} · {task.age}岁</p></div>
